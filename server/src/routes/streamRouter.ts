...
router.post("/exit/stream", (req, res) => {
  ...
  if (ffmpegJob) {
    if (!ffmpegJob.ffmpegJob.killed) ffmpegJob.ffmpegJob.kill();
    res.send("Stream Closed");
  } else res.send("Stream Already Closed");
});
    ...
    if (file.extension() === "mp4") {
      if (!isPlaying) {
        ...
        getVideoMetaData(path.toString()).then((data) => {
          ...
          const segListPath =
            os.homedir() +
            `/CurseFileServer/stream/${generateRandomString(20)}.m3u8`;
          const segWildcard = `${os.homedir()}/CurseFileServer/stream/${generateRandomString(
            20
          )}`;
          ...
          const ffmpeg = spawn("ffmpeg", [
            ...
            "-vf",
            `scale=-2:${
              quality ? quality : getMaxQuality(maxWidth, maxHeight)
            }`,
            //   '-seg_duration', '10',
            // '-use_template', '1' ,
            //  '-use_timeline' ,'0',
          ...
          ffmpeg.on("exit", (code, signal) => {
            if (jobItem) {
              console.log(
                `ffmpeg process exited with code ${code} and signal ${signal} jobQueue ` +
                  hlsStreamJobQueue.size()
              );
          ...
          isHlsFileReady(segListPath, segWildcard)
            .then(() => {
              res
                .status(200)
                .send({
                  m3u8: new Path(segListPath).fileName().toString(),
                  video_meta: data,
                });
            })
            .catch(() => {
...
function deleteStreamSegments(
  ...
  return new Promise<void>(() => {
    ...
    let counter = 0;
    while (true) {
      var path = `${segWildcard}_${"0".repeat(
        3 - counter.toString().length
      )}${counter}.ts`;
